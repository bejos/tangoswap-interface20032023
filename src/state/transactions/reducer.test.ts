...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.COREDAO]).toBeTruthy()
      expect(txs[ChainId.COREDAO]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.COREDAO]?.['0x0']
  ...
  describe('finalizeTransaction', () => {
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.COREDAO_AMBER]?.['0x0']
      expect(tx?.summary).toEqual('hello world')
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.COREDAO), String(ChainId.COREDAO_AMBER)])
      expect(Object.keys(store.getState()[ChainId.COREDAO] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.COREDAO_AMBER] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.COREDAO }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.COREDAO), String(ChainId.COREDAO_AMBER)])
      expect(Object.keys(store.getState()[ChainId.COREDAO] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.COREDAO_AMBER] ?? {})).toEqual(['0x1'])
    })
  })
})
